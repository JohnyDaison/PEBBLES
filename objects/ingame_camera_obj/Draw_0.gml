    ...
    if(view_current == source_cam.view)
    {
        ...
        if(surface_exists(surface))
        {
            if(surface_exists(source_cam.view_surface))
            {
                ...
                x1 = floor((floor(x) - __view_get( e__VW.XView, view_current )) * source_cam.zoom_level) - floor(w/2);
                y1 = floor((floor(y) - __view_get( e__VW.YView, view_current )) * source_cam.zoom_level) - floor(h/2);
