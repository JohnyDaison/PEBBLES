...
for (var i = count - 1; i >= 0; i--) {
    var cur_item = self.members[| i];
    if (!instance_exists(cur_item)) {
        ds_list_delete(self.members, i);
    }
}

self.sitting_on_terrain = false;

count = ds_list_size(self.members);
    ...
    if (!self.annihilate) {
        ...
        for (var i = 0; i < count; i += 1) {
            ...
            for (var ii = 0; ii < count; ii += 1) {
                if (i != ii) {
                    var other_item = self.members[| ii];

                    var speed_size = speed_orig[ii] * coef * (other_item.radius / cur_item.radius / (count - 1));
    ...
    */
    ...
    if (self.annihilate) {
        ...
        */
        ...
        var ready = false;
        if (self.ratio_updated && self.ratio > 0) {
            ready = true;
        }

        if (!self.ratio_updated || self.ratio == 0) {
            self.ratio = 0;
            var baseColorForce = self.force[? self.base_color];

            if (baseColorForce > 0) {
                var inverseColorForce = self.force[? g_white - self.base_color];

                if (inverseColorForce > 0) {
                    self.ratio = baseColorForce / inverseColorForce;
                }
            }

            self.ratio_updated = true;

            /*
            if(abs(1-self.ratio) < self.balance_margin)
                ready = true;
            */
