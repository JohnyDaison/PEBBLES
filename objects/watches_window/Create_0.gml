...
add_watch = function (index) {
    ...
    if (is_undefined(index)) {
        pane = watches_scroll_list.add_item();
    } else {
        pane = watches_scroll_list.insert_item(index);
    }
...
update_watches_values = function () {
    ...
    for (var index = 0; index < total_count; index++) {
        ...
        var parsedValue = parse_stringvalue("object", watch_pane.object_input.text);
        var object = noone;

        if (is_string(parsedValue)) {
            watch_pane.object_input.text = parsedValue;
            object = parse_stringvalue("object", parsedValue);
        } else {
            object = parsedValue;
        }
        ...
        var temp_value = variable_instance_get(inst_id, variable);
