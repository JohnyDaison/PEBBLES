...
is_my_guy_los_blocked = function (rel_x, rel_y) {
    ...
    var next_guy_x = self.my_guy.x + self.my_guy.hspeed + self.center_offset_x;
    var next_guy_y = self.my_guy.y + self.my_guy.vspeed + self.center_offset_y;

    var current_x = next_guy_x + rel_x;
    var current_y = next_guy_y + rel_y;

    var terrain = collision_line(next_guy_x, next_guy_y, current_x, current_y, solid_terrain_obj, false, true);
    if (terrain != noone) {
        blocked = true;
    }

    if (!blocked) {
        var field = collision_line(next_guy_x, next_guy_y, current_x, current_y, gate_field_obj, false, true);
        if (field != noone && (self.holographic || !field.holographic)) {
            blocked = true;
        }
    }

    if (!blocked) {
        var body = collision_line(next_guy_x, next_guy_y, current_x, current_y, phys_body_obj, false, true);
        if (body != noone && body.my_player.team_number != self.my_player.team_number && (self.holographic || !body.holographic)) {
            blocked = true;
        }
    }
