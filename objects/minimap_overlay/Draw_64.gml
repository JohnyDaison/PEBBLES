...
with(big_projectile_obj)
{
    with(other)
    {
        ...
        minimap_x = minimap_view_left + floor(other.x/32 * mini_block_size) - floor(__view_get( e__VW.XView, 0 )/32)*mini_block_size;
        minimap_y = minimap_view_top + floor(other.y/32 * mini_block_size) - floor(__view_get( e__VW.YView, 0 )/32)*mini_block_size;
    ...
    if(endpoint_reached && !invalid)
    {
        with(other)
        {
            ...
            if(!other.collided)
            {
                ...
                minimap_start_x = minimap_view_left + start_x/32 * mini_block_size - floor(__view_get( e__VW.XView, 0 )/32)*mini_block_size -1;
                minimap_start_y = minimap_view_top + start_y/32 * mini_block_size - floor(__view_get( e__VW.YView, 0 )/32)*mini_block_size -1;
            ...
            else
            {
                for(i=1;i<=other.last_node;i+=1)
                {
                    ...
                    {
                        prev_node_x = minimap_view_left + floor(prev_node.x/32 * mini_block_size) - floor(__view_get( e__VW.XView, 0 )/32)*mini_block_size;
                        prev_node_y = minimap_view_top + floor(prev_node.y/32 * mini_block_size) - floor(__view_get( e__VW.YView, 0 )/32)*mini_block_size;
                                        
                        node_x = minimap_view_left + floor(node.x/32 * mini_block_size) - floor(__view_get( e__VW.XView, 0 )/32)*mini_block_size;
                        node_y = minimap_view_top + floor(node.y/32 * mini_block_size) - floor(__view_get( e__VW.YView, 0 )/32)*mini_block_size;
    ...
    for(i=1; i<=match_obj.player_count; i+=1)
    {
        ...
        {        
            ...
            minimap_player_view_left = minimap_view_left
                                       ...
                                       - floor(view_xview[0]/32)* mini_block_size;
            ...
            */
            ...
            minimap_player_x = minimap_view_left + floor( (guy.x-6)/(32/mini_block_size) )
                                                     - floor(__view_get( e__VW.XView, 0 )/32) * mini_block_size;
            minimap_player_y = minimap_view_top + floor( (guy.y-18)/(32/mini_block_size) )
                                                     - floor(__view_get( e__VW.YView, 0 )/32) * mini_block_size;
            /*
