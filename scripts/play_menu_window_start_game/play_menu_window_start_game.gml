...
function play_menu_window_start_game() {
	...
	*/
	...
	// old mods vars
	//gamemode.ban_respawn = mod_controls[? "one_death"].checked;
	//gamemode.ban_base_crystals = !mod_controls[? "base_crystals"].checked;
	//gamemode.ban_cannons = !mod_controls[? "cannons"].checked;
	//gamemode.ban_turrets = !mod_controls[? "turrets"].checked;
	//gamemode.ban_mob_portals = !mod_controls[? "mob_portals"].checked;

	//gamemode.ban_black = !mod_controls[? "black_color"].checked;
	//gamemode.ban_abilities = !mod_controls[? "abilities"].checked;
	//gamemode.ban_tut_guide = !mod_controls[? "tut_guide"].checked;

	//gamemode.hp_death = mod_controls[? "hp_death"].checked;
	//gamemode.holographic_spawners = mod_controls[? "holographic_spawners"].checked;
	//gamemode.weak_terrain = mod_controls[? "weak_terrain"].checked;
	//gamemode.indestr_terrain = mod_controls[? "indestr_terrain"].checked;

	//gamemode.random_item_spawner = mod_controls[? "random_item_spawner"].checked;
	//gamemode.snakes_on_a_plane = mod_controls[? "snakes_on_a_plane"].checked;
	//gamemode.bolt_rain = mod_controls[? "bolt_rain"].checked;
	//gamemode.orbs_energy_lock = mod_controls[? "orbs_energy_lock"].checked;
	//gamemode.orbs_energy_min_lock = mod_controls[? "orbs_energy_min_lock"].checked;
	//gamemode.heavy_shots_mod = mod_controls[? "heavy_shots"].checked;

	//gamemode.slime_mob_rain = mod_controls[? "slime_mob_rain"].checked;
	//gamemode.create_artifacts = mod_controls[? "artifacts"].checked;
	//gamemode.lightning_strikes_mod = mod_controls[? "lightning_strikes"].checked;

	//gamemode.tutorials = mod_controls[? "tutorials"].checked;


	// LIMITS
	var i, limit_id, gm_limits = gm[? "limits"];

	//if(gamemode.object_index == match_obj)
	//{
