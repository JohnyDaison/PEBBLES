function merge_rules_maps(target_map, source, type) {
    ...
    if(!is_undefined(source)) {
        ...
        while(!is_undefined(rule_id))
        {
            ...
            if (type == "default") {
                target_map[? rule_id] = rule_default_value_update(gmRule, target_map[? rule_id], source_map[? rule_id]);
            } else if (type == "forced") {
                target_map[? rule_id] = rule_forced_value_update(gmRule, target_map[? rule_id], source_map[? rule_id]);
            }
