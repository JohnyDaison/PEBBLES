    ...
    switch(pseudo_method)
    {
        ...
        case "demo_after_speech":
            ...
            if(selected_subtask == "use_berserk")
            {
                if(!berserk)
                {
                    ...
                    if(my_color == g_red && !wanna_cast && tint_updated && !speaking && nearest_waypoint.waypoint_id == "use_berserk/active") //  && abi_cooldown[? g_cyan] == 0
                    {
        ...
        case "non_demo":
            ...
            if(selected_subtask == "use_ubershield")
            {
                ...
                if(my_color == g_magenta && tint_updated && nearest_waypoint.waypoint_id == "use_ubershield/active" && status_left[? "ubershield"] == 0
                && abi_cooldown[? g_magenta] < (abi_cooldown_length[? g_magenta] - 2 * abi_script_delay[? g_magenta]) )
                {
                    abi_cooldown[? g_magenta] = 0;
            ...
            {
                if(my_color != g_dark && !channeling)
                {
                    wanna_channel = true;
                }
            
                var near_wp_dist = point_distance(x, y, nearest_waypoint.x, nearest_waypoint.y);
                if(demonstration_mode && my_color == g_dark && tint_updated && nearest_waypoint.waypoint_id == "use_rewind/success-taunt" && near_wp_dist < 64)
                {
                    abi_cooldown[? g_dark] = 0;
