function play_summary_update() {
	...
	with(pane)
	{
	    ...
	    var gm = DB.gamemodes[? gamemode_picker.cur_item_id], i, count, place, mod_id, mod_but, dmod, fmod, gmmod;
    
	    if(!is_undefined(gm))
	    {
	        var default_mods_gm = gm[? "default_modifiers"], forced_mods_gm = gm[? "forced_modifiers"], default_mods_place, forced_mods_place;
	        var min_real_players = gm[? "min_real_players"], max_players = gm[? "max_players"], max_bots = max_players - min_real_players;
	        ...
	        if(max_players > 1)
	        {
	            new_text += "1-" + string(max_players) + " players";
	        }
	        else
	        {
	            new_text += "1 player";
	        ...
	        // LIMITS
	        var limits_text = "", gm_limits = gm[? "limits"], limit_count = ds_map_size(gm_limits), limit_id = ds_map_find_first(gm_limits), limit_index;
	    ...
	    var max_line_length = 36, line, line_length, new_line, space_pos, chi, line_break_delay = 0, do_delay;
