function play_anim_set(current_step) {
    var i, step, temp_dur = 0, duration;
        ...
        if(current_step >= 0 && current_step < self.duration_total)
        {
            for(i=0; i<self.steps_total; i++)
            {
                ...
                if(current_step >= temp_dur && current_step < temp_dur+duration)
                {
                    ...
                    var visible_index = step[? "visible_index"];
                    if(is_undefined(visible_index))
                    {
                        visible_index = 0;
                    }
                    ...
                    if(instance_exists(disp_obj))
                    {
                        ...
                        disp_obj.anim_set_steps_total = visible_steps_total[? disp_obj.id];
                        disp_obj.anim_set_current_step = visible_index;
                    }
                }
