function arena_bot3(argument0) {
    if(npc_active || next_phase != 0)
    {
        ...
        var should_shield = potential_color != g_dark
        ...
        if(phase == 1 && status_left[? "suppressed"] > 0 && my_color != g_magenta && !can_shield)
        {
            if(can_go_magenta)
            {
                phase = 0;
            ...
            if(!auto_chosen_orbs)
            {
                ...
                */
                ...
                {        
                    ...
                    {
                        // DARK ATTACK
                        if(potential_color == g_dark && !npc_wanna_shoot_dark_decided && pl_dist < vortex_range*get_orb_list_power_level(orbs_in_use[? g_dark]))
                        {
                            npc_wanna_shoot_dark = random(2) < difficulty/10;
                        ...
                        if(potential_color > g_dark && current_slot > 0)
                        {
                            // COLOR ATTACKS
                            switch(current_slot)
                            {
