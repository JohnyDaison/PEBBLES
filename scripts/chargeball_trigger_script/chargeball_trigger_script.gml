function chargeball_trigger_script() {
	...
	if(instance_exists(my_guy) && my_color >= g_black && !firing && charge > 0)
	{
	    ...
	    var dist = point_distance(0, 0, rel_x, rel_y);
	    ...
	    else
	    {
	        ...
	        else if(!is_guy || my_guy.status_left[? "suppressed"] == 0)
	        {
	            ...
	            if(orb_count == 1 && (!is_guy || has_level(my_guy, "blast_mode", 1)))
	            {
	                ...
	                //inst = instance_create(x,y,big_projectile_obj);
	                inst = create_energy_ball(id, "big_bolt", my_color, charge);
	                //inst.facing_right = sign(rel_x);
	                //inst.my_player = self.my_player;
	                //inst.force = self.charge;
	                //inst.holographic = self.holographic;
	                //my_sound_play(shot1_sound);
	                var coef = max(abs(rel_x), abs(rel_y));
	                inst.direction = point_direction(0,0, rel_x, rel_y);
	            ...
	            if(orb_count == 3 && (!is_guy || has_level(my_guy, "dashwave_mode", 1)))
	            {
	                ...
	                inst.image_angle = point_direction(0,0, rel_x, rel_y);
